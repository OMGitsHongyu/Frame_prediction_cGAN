...
opt = {
   ...
   nThreads = 5,           -- #  of data loading threads to use
...
if opt.network == '' then
  ...
  -- I and C
  dc1 = nn.JoinTable(2, 2)({dx_I, dx_C})  -- rgb + rgb
  dc2 = nn.JoinTable(2, 2)({dc1, dx_C2})  --  dc1 + flowxy 

  dh1 = nn.SpatialConvolution(3 + 2 + 3, nplanes, 5, 5, 2, 2, 2, 2)(dc2)  -- size / 2 64 
  ...
  dout = nn.Sigmoid()(dh6)
  netD = nn.gModule({dx_I, dx_C, dx_C2}, {dout})
...
local fDx = function(x)
   ...
   targets:fill(1)
   local output = netD:forward( {inputs, cond_inputs_coarse, cond_inputs_flow} )
   local errD_real = criterion:forward(output, targets)
   local df_do = criterion:backward(output, targets)
   netD:backward({inputs, cond_inputs_coarse, cond_inputs_flow}, df_do)
...
local fGx = function(x)
   ...
   local df_do = criterion:backward(output, targets)
   netD:updateGradInput({inputs, cond_inputs_coarse, cond_inputs_flow}, df_do)
