...
-- local trainCache = paths.concat(cache, 'trainCache_full_1frame.t7')
-- local trainCache = paths.concat(cache, 'trainCache_full_5frame.t7')
-- local trainCache = paths.concat(cache, 'trainCache_full_10frame.t7')
local trainCache = paths.concat(cache, 'testCache_full_1frame.t7')
...
function makeData_video(fine, fine2, labels)

   local coarse_size = opt.scale_coarse
   local sample_num  = (#(fine))[1]
   local channel_num = (#(fine))[2]
   local coarse_input0 = torch.Tensor(sample_num, channel_num, coarse_size, coarse_size)
   local label_input = torch.Tensor(sample_num, opt.classnum):fill(0)
   ...
   for i = 1, sample_num do
     ...
     local t_coarse_input = image.scale(now_fine, coarse_size, coarse_size):clone()
     coarse_input0[i] = t_coarse_input:clone()
     local nowlabel = labels[i]
     label_input[i][nowlabel] = 1
   ...
   opt.flag  = 0
   return {fine2,  coarse_input0, label_input}
end
   ...
   for i = 1, sample_num do
     ...
     flow_input[{{i}, {2}, {}, {}}]:copy(now_flowy) 
       ...
       if opt.flag == 1  then
        ...
        saveData(fine2[i]:clone(), hpatch_name)
        saveData(coarse_input0[i]:clone(), coarse_input_name)
...
local trainHook = function(self, imgpath, lblnum, lblid)
   collectgarbage()
