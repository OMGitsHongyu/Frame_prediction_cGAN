...
if opt.network == '' then
  ...
  dh1 = nn.SpatialConvolution(3, nplanes, 5, 5, 2, 2, 2, 2)(dx_I)  -- size / 2 64 
  ...
  c1 = nn.JoinTable(2, 2)({ x_I, x_C })
  h1 = nn.SpatialConvolution(4, nplanes, 5, 5, 2, 2, 2, 2)(c1)  -- 32 * 32
  b1 = nn.SpatialBatchNormalization(nplanes)(h1) 
  r1 = nn.ReLU(true)(b1) 

  h2 = nn.SpatialConvolution( nplanes, nplanes * 2, 5, 5, 2, 2, 2, 2)(r1) -- size / 2  16 * 16
  ...
  h3 = nn.SpatialFullConvolution(nplanes*4, nplanes * 4, 4, 4, 2, 2, 1, 1)(r2_2) -- size * 2
...
local noise_inputs = torch.Tensor(opt.batchSize, 1, opt.scale_coarse, opt.scale_coarse)
local cond_inputs = torch.Tensor(opt.batchSize, opt.condDim[1], opt.condDim[2], opt.condDim[3])
local cond_inputs2 = torch.Tensor(opt.batchSize, opt.classnum, 1, 1)
...
if opt.gpu > 0 then
   ...
   cond_inputs = cond_inputs:cuda();  cond_inputs2 = cond_inputs2:cuda(); targets_fake = targets_fake:cuda()
...
local fDx = function(x)
   ...
   -- cond_inputs:copy(inputs_all[2])
   -- cond_inputs2:copy(inputs_all[3])
   -- targets:copy(inputs_all[4]) 
   cond_inputs_coarse:copy(inputs_all[3])

   targets:fill(1)
   local output = netD:forward(inputs)
   local errD_real = criterion:forward(output, targets)
   local df_do = criterion:backward(output, targets)
   netD:backward(inputs, df_do)
   ...
   -- cond_inputs:copy(inputs_all2[2])
   -- cond_inputs2:copy(inputs_all2[3])
   -- targets:copy(inputs_all2[4]) 
   cond_inputs_coarse:copy(inputs_all2[3])

   gt_inputs:copy(inputs)

   local fake = netG:forward({noise_inputs, cond_inputs_coarse})
  -- print(fake:size())
   inputs:copy(fake)
   targets_fake:fill(0)
...
local fGx = function(x)
   ...
   local df_do = criterion:backward(output, targets)
   netD:updateGradInput(inputs, df_do)
   local df_dg = netD.gradInput
   ...
   netG:backward({noise_inputs, cond_inputs_coarse}, df_dg)
...
--       paths.mkdir('/nfs.yoda/xiaolonw/torch_projects/models_coco/' .. opt.name .. '/')
       paths.mkdir('/scratch/hongyuz/models_coco_firstlast/' .. opt.name .. '/')
...
--       torch.save('/nfs.yoda/xiaolonw/torch_projects/models_coco/' .. opt.name .. '/' .. epoch .. '_net_D.t7', netD:clearState())
       torch.save('/scratch/hongyuz/models_coco_firstlast/' .. opt.name .. '/' .. epoch .. '_net_G.t7', netG:clearState())
       torch.save('/scratch/hongyuz/models_coco_firstlast/' .. opt.name .. '/' .. epoch .. '_net_D.t7', netD:clearState())
