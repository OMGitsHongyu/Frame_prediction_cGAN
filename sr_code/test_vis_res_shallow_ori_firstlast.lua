...
model_G = torch.load('/scratch/hongyuz/models_coco_firstlast/train_coco_res_dcgan_ori64_sr_ori/40_net_G.t7')
...
local resultfile = '/scratch/hongyuz/models_coco_firstlast/train_coco_res_dcgan_ori64_sr_ori/res_cls.txt' 
local file = torch.DiskFile(resultfile, "w")


function getSamples(dataset, N, beg)
  local resultpath = '/scratch/hongyuz/models_coco_firstlast/train_coco_res_dcgan_ori64_sr_ori/'
  ...
  for i=1,N do
      ...
      img_name = paths.concat(resultpath, string.format('%04d_predict.jpg',i + beg))
